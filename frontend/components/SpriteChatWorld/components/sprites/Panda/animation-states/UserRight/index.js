import React, { useState } from "react";

import useInterval from "../../../../../utils/useInterval";

export const UserRightStand = () => {
  return (
    <svg
      width="56"
      height="56"
      viewBox="0 0 56 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      style={{
        imageRendering: "pixelated",
      }}
    >
      <rect
        width="56"
        height="56"
        transform="matrix(-1 0 0 1 56 0)"
        fill="url(#patternPandaRightStand)"
      />
      <defs>
        <pattern
          id="patternPandaRightStand"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlinkHref="#imagePandaRightStand" transform="scale(0.0178571)" />
        </pattern>
        <image
          id="imagePandaRightStand"
          width="56"
          height="56"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
};

export const UserRight = () => {
  const [state, setState] = useState(0);

  useInterval(() => {
    setState((state + 1) % 2);
  }, 300);

  switch (state) {
    case 0:
      return <UserRight1 />;
    case 1:
    default:
      return <UserRight2 />;
  }
};

function UserRight1() {
  return (
    <svg
      width="56"
      height="56"
      viewBox="0 0 56 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      style={{
        imageRendering: "pixelated",
      }}
    >
      <rect
        width="56"
        height="56"
        transform="matrix(-1 0 0 1 56 0)"
        fill="url(#patternPandaRight1)"
      />
      <defs>
        <pattern
          id="patternPandaRight1"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlinkHref="#imagePandaRight1" transform="scale(0.0178571)" />
        </pattern>
        <image
          id="imagePandaRight1"
          width="56"
          height="56"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}

function UserRight2() {
  return (
    <svg
      width="56"
      height="56"
      viewBox="0 0 56 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      style={{
        imageRendering: "pixelated",
      }}
    >
      <rect
        width="56"
        height="56"
        transform="matrix(-1 0 0 1 56 0)"
        fill="url(#patternPandaRight2)"
      />
      <defs>
        <pattern
          id="patternPandaRight2"
          patternContentUnits="objectBoundingBox"
          width="1"
          height="1"
        >
          <use xlinkHref="#imagePandaRight2" transform="scale(0.0178571)" />
        </pattern>
        <image
          id="imagePandaRight2"
          width="56"
          height="56"
          xlinkHref="data:image/png;base64,"
        />
      </defs>
    </svg>
  );
}
